...
# Import Supervised Execution mode (optional - only if supervised_exec_mode.py is available)
try:
    from supervised_exec_mode import SupervisedExecMode, create_yolo_config
    YOLO_AVAILABLE = True
except ImportError:
    YOLO_AVAILABLE = False
    print("⚠️  Supervised Execution mode not available (supervised_exec_mode.py not found)", file=sys.stderr)
...
class SecretRedactor:
    ...
    PATTERNS = [
        ...
        # GitHub Tokens (all types) - relaxed to catch 20+ chars
        ...
        # Passwords (various formats) - match assignment only, not prose
        (r'(?i)"password"\s*:\s*"[^"]+"', 'PASSWORD_REDACTED'),  # JSON format
        ...
        # Slack Tokens (bot, user, app, workspace, etc.)
        ...
        # Mailgun API Keys (key- prefix with hex chars)
        (r'key-[0-9a-z]{32}', 'MAILGUN_API_KEY_REDACTED'),

        # SendGrid API Keys (SG. prefix, 69 chars total)
        ...
        # Telegram Bot Tokens (digits:alphanumeric format)
        (r'\d{8,10}:[a-zA-Z0-9_-]{35}', 'TELEGRAM_BOT_TOKEN_REDACTED'),

        # ========== Phase 1 Improvements (IF.search Pass 1 findings) ==========

        # GitLab Tokens (DS-23 WebSec)
        (r'glpat-[0-9a-zA-Z_\-]{20}', 'GITLAB_PAT_REDACTED'),
        (r'glrt-[0-9a-zA-Z_\-]{20}', 'GITLAB_RUNNER_REDACTED'),

        # Slack User Tokens (DS-23 WebSec)
        (r'xoxp-\d{10,13}-\d{10,13}-\d{10,13}-[a-zA-Z0-9]{32}', 'SLACK_USER_REDACTED'),

        # Twilio Account SID (DS-23 WebSec - complements existing SK pattern)
        (r'AC[0-9a-fA-F]{32}', 'TWILIO_ACCOUNT_SID_REDACTED'),

        # Service-Specific Context Patterns (DS-23 WebSec - fixes FN #2, #3, #5, #6)
...
yolo = None    # Will be initialized if Supervised Execution mode enabled
...
async def list_tools() -> list[Tool]:
    ...
    # Add Supervised Execution mode tools if available
    if YOLO_AVAILABLE:
        tools.extend([
            Tool(
                name="enable_supervised_exec_mode",
            ...
            Tool(
                ...
                description="Execute a command (requires Supervised Execution mode enabled). Both agents will see the result.",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "conversation_id": {"type": "string", "pattern": "^conv_[a-f0-9]{16}$"},
...
Alive: {status['alive']}"""
        ...
        # Supervised Execution mode tools
        elif name == "enable_supervised_exec_mode" and YOLO_AVAILABLE:
            global yolo
            if yolo is None:
                yolo = SupervisedExecMode(bridge)
...
Results will be visible to both sessions."""
            ...
            if yolo is None:
                return [TextContent(
                    type="text",
                    text="❌ Supervised Execution mode not initialized. Use enable_supervised_exec_mode first."
                )]
